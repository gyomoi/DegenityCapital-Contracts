# Set FORK RPC URL
export ETH_RPC_URL=https://kovan.infura.io/v3/c76efe7d627040729fe2c0e46045aa2c

# deploy
export ETH_FROM=0x00d16F998e1f62fB2a58995dd2042f108eB800d1

# export TREASURY_WALLET=[UPDATE TO MULTISIG ADDRESS TO RECEIEVE ETH SHARES]
# export LIQUIDITY_WALLET=[UPDATE TO LIQUIDITY WALLET TO RECEIVE LP TOKENS]

export TOKEN_ADDRESS=0xbdbAeB6c6b0D351dAa38BB2e87460fEBa514cc61
export ETH_GAS_PRICE=6000000000
export ETH_GAS=$(seth estimate $TOKEN_ADDRESS "removeFromBlacklist(address)" "0xe1Da6dcb8C2e217EfA9fe517F29562688b86F575"  --rpc-url $ETH_RPC_URL)

seth send $TOKEN_ADDRESS "removeFromBlacklist(address)" "0xe1Da6dcb8C2e217EfA9fe517F29562688b86F575" --rpc-url $ETH_RPC_URL